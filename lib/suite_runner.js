...
var TimestamperReporter = require('./reporter/timestamper');
...
var SpecReporter = require('./reporter/spec');
var TestFailureError = require('./test_failure_error');
var TimeoutTimer = require('./timeout_timer');

var defaultInterface = __dirname + '/interface/bdd_mocha';
...
function runTest(softKill, childProcess, timeout, graceTime, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath) {
...
function runTestWithRetries(softKill, childProcess, timeout, graceTime, attempts, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath) {
  ...
  return runTest(softKill, childProcess, timeout, graceTime, slowThreshold, retryingReporter, testInterfacePath, testInterfaceParameter, testPath)
    .catch(function(error) {
      if (attempts <= 1) {
        throw error;
      } else {
        return runTestWithRetries(softKill, childProcess, timeout, graceTime, attempts - 1, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath);
      }
    });
}

function runOrSkipTest(softKill, childProcess, attempts, timeout, graceTime, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testDescriptor) {
  ...
  if (testDescriptor.skipped) {
    ...
    return runTestWithRetries(softKill, childProcess, timeout, graceTime, attempts, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath);
  }
}
...
function listTestsOfFiles(timeout, testInterfacePath, testInterfaceParameter, suites) {
  return when.all(suites.map(function(suite) {
      return listSuite.listTestsOfFile(
        timeout, testInterfacePath, testInterfaceParameter, suite);
    }))
...
function listTests(reporter, timeout, testInterfacePath, testInterfaceParameter, suites) {
  return listTestsOfFiles(timeout, testInterfacePath, testInterfaceParameter, suites)
 ...
 * interface: Path to interface file. Optional, the BDD interface is chosen by default.
 * interfaceParameter: Parameter to the interface. Optional, but some interfaces may require it.
 * reporters: Array of reporter objects. Optional, the spec reporter is chosen by default.
...
module.exports = function(options) {
  ...
  var resultPromise = listTests(reporter, listingTimeout, testInterfacePath, testInterfaceParameter, options.suites)
