...
function runTest(childProcess, timeout, slowThreshold, reporter, testInterfacePath, testPath) {
  reporter.gotMessage(testPath, { type: 'start' });
...
function runTestWithRetries(childProcess, timeout, attempts, slowThreshold, reporter, testInterfacePath, testPath) {
  ...
  return runTest(childProcess, timeout, slowThreshold, reporter, testInterfacePath, testPath)
    .catch(function(error) {
      if (attempts <= 1) {
        throw error;
      } else {
        return runTestWithRetries(childProcess, timeout, attempts - 1, slowThreshold, reporter, testInterfacePath, testPath);
      }
    });
}

function runOrSkipTest(childProcess, attempts, timeout, slowThreshold, reporter, testInterfacePath, testDescriptor) {
  ...
  if (testDescriptor.skipped) {
    ...
    return runTestWithRetries(childProcess, timeout, attempts, slowThreshold, reporter, testInterfacePath, testPath);
  }
}
...
module.exports = function(options) {
  ...
  var resultPromise = listTests(reporter, timeout, testInterfacePath, options.suites)
      ...
      return nodefn.call(async.parallelLimit, tests.map(function(test) {
          return function(done) {
            if (cancelled) {
              ...
              return runOrSkipTest(options.childProcess || childProcess, attempts, timeout, slowThreshold, reporter, testInterfacePath, test)
