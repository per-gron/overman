...
function runTestProcess(childProcess, debugPort, timeout, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath) {
  var parameters = {
    timeout: timeout,
...
function runTest(softKill, timeoutTimer, childProcess, debugPort, timeout, graceTime, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath) {
  var didTimeout = false;
  var child = runTestProcess(childProcess, debugPort, timeout, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath);
...
function runTestWithRetries(softKill, timeoutTimer, childProcess, debugPort, timeout, graceTime, attempts, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath) {
  ...
  function sendPostponsedFinishMessage(changeToRetry) {
    ...
    var message = changeToRetry ? _.extend({}, postponedFinishMessage, { type: 'retry' }) : postponedFinishMessage;
    reporter.gotMessage(testPath, message);
  }

  return runTest(softKill, timeoutTimer, childProcess, debugPort, timeout, graceTime, slowThreshold, retryingReporter, testInterfacePath, testInterfaceParameter, testPath)
    .then(function() {
    ...
    .catch(function(error) {
      ...
      if (attempts <= 1) {
        ...
        return runTestWithRetries(softKill, timeoutTimer, childProcess, debugPort, timeout, graceTime, attempts - 1, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath);
      }
    });
}

function runOrSkipTest(softKill, timeoutTimer, childProcess, debugPort, attempts, globalTimeout, graceTime, globalSlowThreshold, reporter, testInterfacePath, testInterfaceParameter, testDescriptor) {
  ...
  if (testDescriptor.skipped) {
    ...
    return runTestWithRetries(softKill, timeoutTimer, childProcess, debugPort, timeout, graceTime, attempts, slowThreshold, reporter, testInterfacePath, testInterfaceParameter, testPath);
  }
}
