...
function runOrSkipTest(childProcess, timeout, slowThreshold, reporter, testInterfacePath, testDescriptor) {
...
function withTimeout(promise, timeout, onTimeout) {
  return when.race([
    promise,
    when().delay(timeout).then(onTimeout)
  ]);
}

function listTestsOfFiles(timeout, testInterfacePath, suites) {
  return when.all(suites.map(function(suite) {
      return withTimeout(listSuite.listTestsOfFile(testInterfacePath, suite), timeout, function onTimeout() {
        throw new TestFailureError('Timed out while listing tests of ' + suite);
      });
    }))
      ...
      return nodefn.call(async.parallelLimit, tests.map(function(test) {
          return function(done) {
            if (cancelled) {
              ...
              return runOrSkipTest(options.child_process || childProcess, timeout, slowThreshold, reporter, testInterfacePath, test)
