...
function killAfter(softKill, child, timeToWait, willKill) {
...
function runTest(softKill, childProcess, timeout, slowThreshold, reporter, testInterfacePath, testPath) {
  var child = runTestProcess(childProcess, timeout, slowThreshold, reporter, testInterfacePath, testPath);

  var didTimeout = false;
  killAfter(softKill, child, timeout, function() {
    didTimeout = true;
...
function runTestWithRetries(softKill, childProcess, timeout, attempts, slowThreshold, reporter, testInterfacePath, testPath) {
  ...
  return runTest(softKill, childProcess, timeout, slowThreshold, retryingReporter, testInterfacePath, testPath)
    .catch(function(error) {
      if (attempts <= 1) {
        throw error;
      } else {
        return runTestWithRetries(softKill, childProcess, timeout, attempts - 1, slowThreshold, reporter, testInterfacePath, testPath);
      }
    });
}

function runOrSkipTest(softKill, childProcess, attempts, timeout, slowThreshold, reporter, testInterfacePath, testDescriptor) {
  ...
  if (testDescriptor.skipped) {
    ...
    return runTestWithRetries(softKill, childProcess, timeout, attempts, slowThreshold, reporter, testInterfacePath, testPath);
  }
}
