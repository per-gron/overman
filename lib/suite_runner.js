...
function resolveTestInterface(interfacePath) {
  return require(path.resolve(interfacePath));
}
...
function runTest(testInterfacePath, testPath) {
  childProcess.fork(__dirname + '/test_runner.js', [testPath.file].concat(testPath.path));
}
...
module.exports = function(options) {
  var testInterfacePath = options.interface;
  var testInterface = resolveTestInterface(testInterfacePath);
