...
function testsOfSuite(file, suite, suitePath, skipped) {
  suitePath = suitePath || [];
  ...
  if (suite.type === 'test') {
    ...
    return _.flatten(suite.contents.map(function(subSuite) {
      return testsOfSuite(file, subSuite, subPath, skipped || subSuite.skipped);
    }), true);
  } else {
...
function reportSkippedTest(reporter, testPath) {
  reporter.gotMessage(testPath, { type: 'skipped' });
}
...
function runTest(reporter, testInterfacePath, testPath) {
  return when.promise(function(resolve, reject) {
    ...
    killAfter(child, 1000, function() {
      reporter.gotMessage(testPath, { type: 'debugInfo', key: 'overman.timeout', value: true });
    });
...
function runOrSkipTest(reporter, testInterfacePath, testDescriptor) {
  ...
  if (testDescriptor.skipped) {
    ...
    return runTest(reporter, testInterfacePath, testPath);
  }
}
...
module.exports = function(options) {
  ...
  return inParallel(tests.map(function(test) {
      return function() {
        return runOrSkipTest(reporter, testInterfacePath, test);
      };
