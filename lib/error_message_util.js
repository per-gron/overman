 ...
 *     { in: 'test|beforeHook|afterHook', [inName: 'name'] }
 */
...
function prettyError(errorMessage) {
  ...
  if (errorMessage.in) {
    result += colors.errorPlace('In ' + prettyErrorLocation(errorMessage) + ':') + ' ';
  }
...
function prettyTimeout(location) {
  return (colors.errorPlace('In ' + prettyErrorLocation(location) + ':') + ' ' +
