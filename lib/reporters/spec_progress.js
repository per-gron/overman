...
SpecProgress.prototype._pipeStream = function(stream, id) {
  if (!stream) {
    return;
  }

  var self = this;
  var lines = readline.createInterface({ input: stream, output: through() });
...
SpecProgress.prototype.gotMessage = function(testPath, message) {
  ...
  if (message.type === 'suiteStart') {
    ...
    this._pipeStream(message.stdout, pathAsString);
    this._pipeStream(message.stderr, pathAsString);
