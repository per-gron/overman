...
function incrementValueForSuitePath(obj, suitePath, by) {
  if (suitePath.path.length === 0) {
    ...
    incrementValueForSuitePath(obj, suitePathOf(suitePath), by);
  }
}
...
function calculateNumTestsForSuites(testPaths) {
  ...
  testPaths.forEach(function(testPath) {
    incrementValueForSuitePath(result, suitePathOf(testPath));
  });
...
Serializer.prototype._currentTestFinished = function() {
  incrementValueForSuitePath(this._numRemainingTests, suitePathOf(JSON.parse(this._currentTest)), -1);
  this._canPickNewTest = true;
};
...
Serializer.prototype._getPremissibleSuiteForNextTest = function(currentSuite) {
  if (currentSuite.path.length === 0) {
    ...
    return this._getPremissibleSuiteForNextTest(suitePathOf(currentSuite));
  }
};
...
Serializer.prototype._getPermissibleNextTests = function() {
  ...
  var permissibleSuiteForNextTest = this._currentTest && this._getPremissibleSuiteForNextTest(suitePathOf(JSON.parse(this._currentTest)));
