...
function invokeGeneratorOrPromiseFunction(fun) {
  ...
  if (result && result.next) {  // This looks like a generator
    return when.promise(function(resolve) {
      // fixme: the promise returned by co seems to be
      //        incompatible with the ones that when uses.
      co(result).then(resolve);
    });
  } else {
...
function runTest(foundTest, runAfter) {
  ...
  // sure to time out.
  return makeSureProcessRunsUntilPromiseIsFulfilled(when.resolve()
    .then(function() {
...
process.on('message', function(message) {
  ...
  if (message.type === 'sigint') {
    runAfterHooksOnce().done(end, end);
  }
});
