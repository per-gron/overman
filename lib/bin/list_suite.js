...
function only(object, keys) {
  return _.omit(object, function(value, key) { return !_.contains(keys, key); });
}

var propertyKeys = ['skipped', 'only'];
...
function testsOfSuite(file, suite, suitePath, properties) {
  ...
  if (suite.type === 'test') {
    ...
    return _.flatten(suite.contents.map(function(subSuite) {
      return testsOfSuite(file, subSuite, subPath, _.extend(only(subSuite, propertyKeys), properties));
    }), true);
  } else {
